...
export function CreateRaffleDialog({ onCreate }: CreateRaffleDialogProps) {
  ...
  const PLATFORM_PIX_KEY = "pagamentos@rifazap.com";
  ...
  const handleFinalize = () => {
    setLoading(true);
    setTimeout(() => {
      if (onCreate && pendingRaffle) onCreate(pendingRaffle);
      setOpen(false);
      setStep('form');
      ...
      toast({
        ...
        description: "Sua campanha já está ativa e pronta para receber reservas.",
      });
    }, 1500);
  };

  const copyPlatformPix = () => {
    navigator.clipboard.writeText(PLATFORM_PIX_KEY);
    toast({
      title: "Chave PIX copiada!",
      description: "Agora basta pagar os R$ 9,90 no seu banco.",
    });
  };
          ...
          <div className="p-6 flex flex-col items-center space-y-6 text-center">
            ...
            <div className="w-full space-y-2">
              ...
              <div className="flex items-center gap-2 bg-muted p-4 rounded-xl font-mono text-sm break-all border group">
                {PLATFORM_PIX_KEY}
                <Button variant="ghost" size="icon" onClick={copyPlatformPix} className="shrink-0 hover:bg-primary/20">
